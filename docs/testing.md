...
- Playwright: Reviews CRUD/privacy, official navigation color/geometry and hover-only labels, both-extension dedup/remount, fully transparent body-level control hover reveal, timestamp capture, owned pause/resume, color/rating create-edit persistence, centered SVG close control, outside/Escape/X dismissal, immersed marker hiding, clustering, marker positioning/seeking, and pointer-transparent layer passed.
- Real WebView2 enabled: official URL plus dynamic server URL, frozen runtime, one Reviews item, and one Timestamp Notes item.
- Real official-player CDP smoke: generated H.264/AAC playback plus valid episode metadata produced both player buttons, persisted a custom marker color/rating, dismissed the popover outside and through its close button, hid marker UI while immersed, revealed the fully transparent pointer-active dock on hover, matched the official main-navigation button/icon geometry, color, and hover label, and kept the body-owned marker layer aligned to the live seek slider with no extension console failures.
