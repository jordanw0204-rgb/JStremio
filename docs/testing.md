...
- TypeScript: strict typecheck and 13 unit tests passed, including the current official focusable-`div` controls, immersed-player signal, and layered-slider fixture.
- Playwright: Reviews CRUD/privacy, both-extension dedup/remount, body-level control ownership and hover reveal, timestamp capture, owned pause/resume, color/rating create-edit persistence, outside/Escape/X dismissal, immersed marker hiding, clustering, marker positioning/seeking, and pointer-transparent layer passed.
- Real WebView2 enabled: official URL plus dynamic server URL, frozen runtime, one Reviews item, and one Timestamp Notes item.
- Real official-player CDP smoke: generated H.264/AAC playback plus valid episode metadata produced both player buttons, persisted a custom marker color/rating, dismissed the popover outside and through its close button, hid marker UI while immersed, revealed the faint pointer-active dock on hover, and kept the body-owned marker layer aligned to the live seek slider with no extension console failures.
