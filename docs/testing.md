...
- JStremio native layer: 33 tests passed, including first write, Windows replacement/backup, malformed/unsupported preservation, concurrency, abandoned temps, validation, backward-compatible note customization, constrained native-frame thumbnail capture, CRUD, UUIDs, manifests, origins, fixed IPC, and the pinned shell handshake.
...
- Playwright: Reviews CRUD/privacy, Shadow DOM typing isolation from Stremio shortcuts, official navigation color/geometry and hover-only labels, both-extension dedup/remount, fully transparent body-level control hover reveal, timestamp capture, optional frame thumbnails in management and marker popovers, pinned click-popover positioning, owned pause/resume, paused shell-fullscreen marker realignment, color/rating create-edit persistence, centered SVG close control, outside/Escape/X dismissal, immersed marker hiding, clustering, marker positioning/seeking, and pointer-transparent layer passed.
